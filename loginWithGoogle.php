...
if(!empty($jsonObj->request_type) && $jsonObj->request_type == 'user_auth') {
    ...
    $phone = !empty($jsonObj->phone) ? $jsonObj->phone : '';
    ...
    if(!empty($responsePayload)) {
        ...
        if($stm->rowCount() > 0) {
            ...
            $sql = "UPDATE `partner` SET `PARTNER_NAME` = :namep, `PARTNER_PHONE` = :phone, `PARTNER_STATE` = 'ACTIVE' WHERE `PARTNER_EMAIL` = :email";
            $stm = $db->prepare($sql);
            ...
            // Insert user data
            $sql = "INSERT INTO `partner`(PARTNER_NAME, PARTNER_EMAIL, PARTNER_PHONE, PARTNER_STATE) VALUES (:namep, :email, :phone, 'ACTIVE')";
            $stm = $db->prepare($sql);
            $stm->bindParam(":namep", $name);
            $stm->bindParam(":email", $email);
            $stm->bindParam(":phone", $phone);
